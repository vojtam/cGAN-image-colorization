...
def process_mappings(mapping, reverse_mapping, all_confs):
	...
	for k in mapping.keys():
		...
		if len(matches) == 0:
			...
			FN_thresh = np.max([match[2] for match in matches]) # As soon as the first match appears, it is no longer a FN
		...
		if len(matches) != 0:
			for match in matches:
				...
				if rank == 0:
					...
					all_match_confs = [ref_matches[ordering[ridx]][2] for ridx in np.arange(rank-1, -1, -1)] # Find the confidences of all the matches with higher IoU
					min_match_conf = np.min(all_match_confs)
					if min_match_conf > all_confs[k]: # There is a better match that appears before this one. For this ref box, it's a FP.
						pass
					else:
						FP_thresh = min(FP_thresh, min_match_conf) # This match is the first until another one appears later. What is the lowest confidence for which this is still not a FP?
